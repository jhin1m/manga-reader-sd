...
The CommentSection component is integrated into the manga detail page to show a unified view of all discussions (manga-level and chapter-level):

```typescript
// Key integration pattern (Phase 02 refinement)
const { data: commentsData } = useMangaComments(manga.slug, {
  page: commentPage,
  sort: commentSort,
  type: "all", // Unified view
});
```

**Features implemented:**

- Displays unified discussion feed (manga + all chapters)
- Chapter comments feature explicit badges (e.g., "Chapter 1095")
- Manga-level comments have no badge for clean distinction
...
**File**: `components/comments/chapter-reader-comments.tsx` (via `ReaderView`)

Simplified tab-based discussion system:

- **Chapter Comments**: Discussions specific to the current chapter.
- **All Comments**: Unified feed of all manga and chapter comments for the series.

**Key features:**

- Reduced complexity (2 tabs instead of 3)
- Automatic sorting by date in the "All Comments" tab
- Unified badge logic for context (Phase 03)
